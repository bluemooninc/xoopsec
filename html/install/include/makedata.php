...
function make_data(&$dbm, &$cm, $adminname, $adminpass, $adminmail, $language, $gruops, $timezone){
	...
	$dbm->insert('config', " VALUES (7, 0, 1, 'theme_set', '_MD_AM_DTHEME', 'xoopstrap', '_MD_AM_DTHEMEDSC', 'theme', 'other', 12)");
	...
	$dbm->insert('config', " VALUES (72, 0, 1, 'theme_set_allowed', '_MD_AM_THEMEOK', '".serialize(array('xoopstrap'))."', '_MD_AM_THEMEOKDSC', 'theme_multi', 'array', 13)");
	...
	$temp = md5($adminpass);
	$regdate = time();
	$dbm->insert('users', " VALUES (1,'','".addslashes($adminname)."','".addslashes($adminmail)."','".XOOPS_URL."/','blank.gif','".$regdate."','','','',1,'','','','','".$temp."',0,0,7,5,'xoopstrap','".$time_diff."',".time().",'thread',0,1,0,'','','',0)");
