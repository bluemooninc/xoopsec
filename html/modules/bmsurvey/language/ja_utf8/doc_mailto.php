...
これは、「アンケートを編集する」のGeneralで回答結果を送信するメールアドレスとは別に設定可能です。
このmailtoオプションを使用すれば、通常のお問い合わせアンケートと同様の入力画面が作成可能です。

<h2>１：セクションテキストに mailto を記述する</h2>

<li>アンケートの編集にて、「新しいフィールド」をクリックします。
...
ノーの場合は、アンケートの一般設定に指定したメールアドレスかＸＯＯＰＳの管理者アドレスが適用されます。
...
<li>これで、メールアドレスとサブジェクトが固定でメッセージだけ任意入力のメールアンケートとして利用可能になります。

<h2>４：入力デフォルト値の設定</h2>
モジュール管理のアンケート管理タグより「アンケートの結果を見る」にて回答者リストボックスから回答結果を見る事ができますが、
ここで「この投稿を入力初期値にセットする」をクリックするとデフォルトの入力値に設定できます。
また、解除は「アンケートを編集する」からアンケートを選びGeneralタブ項目Default Responseを空欄にします。

<hr>
<h1>応用例</h1>

<h2>１：メーリングリストへの登録</h2>
以下の設定によりメーリングリストへの登録アンケートとして利用可能です。
メール送信元はメッセージ欄が、'mailfrom'で入力されたメールアドレスとなります。（メッセージの内容は送信されません。）
<p>
<li>アンケートの編集'General'のEmail欄をメーリングリストの登録受付アドレスに指定します。また、From Option はアンケート中のemail欄とします。
<li>メールの送信元は、フィールドネーム'mailfrom'をテキスト・ボックスとして追加します。
